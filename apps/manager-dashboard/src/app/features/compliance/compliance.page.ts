...
@Component({
  ...
  styles: [`
    ...
    .summary-card.total .card-icon { background: #eef4ff; color: #4f8cff; }
    .summary-card.high .card-icon { background: #fef2f2; color: #ef4444; }
    .summary-card.high .card-value { color: #dc2626; }
    .summary-card.medium .card-icon { background: #fffbeb; color: #f59e0b; }
    .summary-card.medium .card-value { color: #d97706; }
    .summary-card.low .card-icon { background: #f0fdf4; color: #22c55e; }
    .summary-card.low .card-value { color: #16a34a; }
...
export class CompliancePage {
  ...
  sendAlert(risk: ComplianceRisk): void {
    console.log('Send alert for risk:', risk.id, risk.employeeName, risk.riskType);
    // Stub: would trigger notification service
  }

  viewEmployee(risk: ComplianceRisk): void {
    console.log('View employee:', risk.employeeId, risk.employeeName);
    // Stub: would navigate to employee detail
  }
}
